...
public class SleepTask extends Task {
    ...
    private final Vec3i BED_CLEAR_SIZE = new Vec3i(4, 3, 3); //increased the clearsize because of "the bed is obstructed"
    private final Vec3i[] BED_BOTTOM_PLATFORM = new Vec3i[]{
            new Vec3i(0, -1, 0),
            new Vec3i(1, -1, 0),
            new Vec3i(2, -1, 0),
            new Vec3i(0, -1, 1), //added some blocks to the platform
            new Vec3i(1, -1, 1),
            new Vec3i(2, -1, 1),
            new Vec3i(0, -1, 2),
            new Vec3i(1, -1, 2),
    ...
    private final Vec3i BED_PLACE_STAND_POS = new Vec3i(-1, 0, 1);//Move 1 block away from the bed, because it can't place it sometimes
    ...
    private final TimerGame _inBedTimer = new TimerGame(10);
    private final TimeoutWanderTask _wanderTask = new TimeoutWanderTask(15, true);
    ...
    protected Task onTick(AltoClef mod) {
        ...
        if (mod.getBlockTracker().anyFound(BEDS)) {
            ...
            return new DoToClosestBlockTask(toSleepIn -> {
                boolean closeEnough = toSleepIn.isWithinDistance(mod.getPlayer().getPos(), 2);
                boolean notTooFar = toSleepIn.isWithinDistance(mod.getPlayer().getPos(), 12);
                if (notTooFar) { // If the bed is near
                    if (closeEnough) {
                        // why 0.2? I'm tired.
                        Vec3d centerBed = new Vec3d(toSleepIn.getX() + 0.5, toSleepIn.getY() + 0.2, toSleepIn.getZ() + 0.5);
                        BlockHitResult hit = LookHelper.raycast(mod.getPlayer(), centerBed, 3);
                ...
                return placeBed(mod); //Place a bed since the nearest is far away
            }, BEDS);
        }
	return placeBed(mod); //Place a bed since there is none nearby
    }
